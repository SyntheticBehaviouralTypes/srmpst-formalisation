...
-- (TODO.md Step 6) over `Check.Sets` with the same signatures the deleted
...
module Check.Graph (N : ℕ) where
  ...
  typecheck :
    ...
    → Dec (ProcessTyping (underlying (proj₁ WR)) (wb-of WR) [] P Pr
             (initial (proj₁ WR)))
  typecheck WR P Pr =
    SetCheck.tc? (underlying (proj₁ WR)) (wb-of WR) [] [] P Pr
      (initial (proj₁ WR))
  ...
  typecheckSession WR M =
    SetCheck.tcSession? (underlying (proj₁ WR)) (wb-of WR) M
      (initial (proj₁ WR))
