...
module Check.Network (N : ℕ) where
  ...
  open Check.Sets N using (module SetCheck)
  ...
  -- Restored (TODO.md Step 6) over `Check.Sets`.  Note the checker runs on
  ...
  typecheckNet {n} w P Pr =
    SetCheck.tc? (underlying (present n)) (wb-net w) [] [] P Pr
      (initial (present n))
  ...
  typecheckSessionNet {n} w M =
    SetCheck.tcSession? (underlying (present n)) (wb-net w) M
      (initial (present n))
