...
-- (`WBGraph`) and its smart constructor (`buildG`) — no module to
-- instantiate first. The participant count `N` is shared, unqualified,
-- across this whole file via an anonymous parameterised module
-- (`module _ {N} where`); from the outside it surfaces as an ordinary
