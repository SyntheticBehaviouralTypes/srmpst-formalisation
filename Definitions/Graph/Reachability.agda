...
module Definitions.Graph.Reachability (N : ℕ) where
  ...
  module _ (G : Graph) where
    ...
    -- `here px` only says the *first* action of the trace matches
    -- `P ∈α_` — it says nothing about the tail (that's only forced for
    -- terms built via the `in/α` smart constructor specifically, not for
    -- an arbitrary inhabitant of the `P ∈T s` existential), so the tail
    -- run/list are discarded (`_`) rather than assumed to be
    -- `tr/refl`/`[]`.
    ∈T→reach :
      ∀ {P s} → P ∈T s
